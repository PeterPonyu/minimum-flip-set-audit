%% Working draft. Structure is generated; prose is authored here.
%% Build from the repository root with: bash scripts/build_paper.sh <this paper dir>
\documentclass[11pt]{article}

\usepackage[utf8]{inputenc}
\usepackage[T1]{fontenc}
\usepackage{amsmath,amssymb,amsthm}
\usepackage{booktabs}
\usepackage{graphicx}
\usepackage{tikz}
\usepackage[margin=1in]{geometry}
% Suppress non-rendered local compilation metadata from the PDF.
\pdfsuppressptexinfo=-1
\usepackage[hidelinks]{hyperref}
\hypersetup{pdfkeywords={fragility index, paired comparison, evaluation, statistical power, annotation disagreement, retrieval-augmented generation, negative result}}
\numberwithin{equation}{section}

\graphicspath{{../figs/out/}}

\renewcommand{\topfraction}{0.9}
\renewcommand{\bottomfraction}{0.7}
\renewcommand{\textfraction}{0.08}
\renewcommand{\floatpagefraction}{0.8}
\setcounter{topnumber}{2}
\setcounter{totalnumber}{3}

%% Numbers and tables are emitted by the analysis code from hash-bound evidence.
\input{generated_numbers}
\input{captions}
\input{theorems}

\title{Count the gradings, not the questions: how far each conclusion of a small
paired evaluation is from being overturned}
\author{Anonymous authors}
\date{}

\begin{document}
\maketitle

\begin{abstract}
\input{abstract}
\end{abstract}

\paragraph{Keywords} fragility index, paired comparison, evaluation, statistical power, annotation disagreement, retrieval-augmented generation, negative result

\input{body}

\bibliographystyle{unsrt}
\bibliography{references}

\end{document}
