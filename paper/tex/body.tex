...
re-hashed on every build, yet no figure reads it and it is withheld from the
public tree, because it is annotated with an internal label and carries a value
its own record marks as not to be used. Binding an artifact the analysis
deliberately declines to read may look redundant. It is the opposite: it is the
only way the decision to decline leaves a trace. An artifact that is simply
never mentioned is indistinguishable from one nobody noticed, whereas this row
records that the value existed, was seen, and was refused.
