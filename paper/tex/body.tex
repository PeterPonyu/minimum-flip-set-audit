...
Nothing here regenerates those answers. The rows are on disk, the recorded
contrasts are on disk, and the objects added here are counts, identities, and a
simulation: for each conclusion, the smallest number of individual gradings that
would have to have come out the other way for it to stop holding; a closed form
that matches the search on every four-cell state of the three designs; the same
three nested string rules counted on all \Answers{} answers rather than on the
image subset; and the operating characteristic of that count under a paired
Bernoulli model.
...
\SigK{}. The prespecified kill for
...
by three nested string rules finds \RegradeContested{} answers they disagree
about. One of those \RegradeContested{} is among the \HarmCandidates{} that
carry the harm contrast: applied as the record states it, the reported
difference moves from \HarmDiff{} to \RegradedDiff{} percentage points.
The conclusion and its negation are separated by one adjudication of one string.
The other three sit on year-valued long-tail answers and, under the
numeric-tolerant rule, are enough to drop the Holm-surviving long-tail text pair
out of the family of \Contrasts{}.
...
not call it significant --- needs \HelpVerdictK{}. On
...
this line of work would be measured against was never obtained; that comparison
is recorded as \PrimaryTier{}, the comparison against the strongest published
method that would follow it as \SotaTier{}, and the counting below does not stand
in for either. The
contribution is an application of an established fragility construction to a
paired grading unit. At this scale a conclusion is a property of individual
gradings, and a results table that prints a difference and a p-value while
omitting how many gradings each rests on has withheld the part a reader would
need in order to know what they are looking at. This is not a new statistical
family.
...
not a perturbation count, and it is not the statistic applied here.
...
conclusion depends on. That is the reporting gap this application addresses.
It transfers an established fragility-index construction to a paired grading
unit; it is not a new statistical family. The count
is complementary rather than competing: it is read beside the
...
contrast actually depends on. The present paper does not introduce a new
retriever. It asks that prior question of one already-recorded comparison.
...
this scale, and it is also the least interesting one available: it says that
supplying a fact makes a model more likely to state that fact.
...
The flip ladder counts how many individual gradings could alter a conclusion;
the same contrasts can also be read on their accuracy scale. Fig.~\ref{fig:effect-forest}
...
tables, not a second test: the compact key suppresses item text, and $m$ makes
the number of informative questions visible next to the percentage-point
difference. The spread of intervals is therefore read together with the flip
counts below, rather than as evidence for a new retrieval effect.
...
\subsection{The nested rules disagree on \RegradeContested{} answers, not \RegradeImageFlip{}}
...
keys, answer strings, and gold values remain in the private evidence rather
than the publication source. The image-subset disagreement is one of the
...
read one numeric string. Neither rule is endorsed here.

The same three rules are not independent graders. They are nested string
matches: exact token, substring, then a numeric window that also fires on year
answers. Two local instruction-model judges, scoring the stored strings and
nothing else, agree with the stored rule at Cohen's \(\kappa=\LLMKappaQwen{}\)
and \(\kappa=\LLMKappaLlama{}\), and with each other at
\(\kappa=\LLMKappaBoth{}\). That agreement is a consistency check on the stored
strings, not an independent rating of the answers: the judges read the same
strings the rules read, and what they confirm is that the stored rule's verdicts
are the ones a reader of those strings would also reach, not that it is the
right rule.
...
Where a conclusion's distance is one, the private evidence can enumerate every
candidate grading, but the publication surface reports only the count.
Table~\ref{tab:candidates} gives that aggregate count for both such conclusions
and explicitly marks the row identities as withheld. This preserves the
fragility result without publishing question keys, answer strings, or gold
values. A reader can still audit the count from an approved, separately
redacted evidence release; no row-level material is required to interpret the
figure or the conclusion.
...
contrast has \HelpVerdictCandidates{} such answers. Neither count is a
confidence guarantee: it says how many distinct adjudications could carry the
conclusion, not which private row produced each one.
...
that a correction would raise the bar; this applies it. The survivors are all
differences against no retrieval at all, and none of them is a comparison
between the two retrieval conditions, which is the comparison the evaluation
was built to make. Two of those \SurvivesHolm{} rows are the
same long-tail text table counted twice. Under the paper's own numeric-tolerant
nested rule that table moves from Holm-adjusted \(p=\SigHolm{}\) to
...
about the grading rule and the family definition, not a property of the
evaluation alone.
...
large. That is an operating characteristic of the statistic, a description of
how the verdict distance behaves at small \(n\), not a finding about the
evaluation.

\begin{figure}[tbp]
\centering
\includegraphics[width=0.72\linewidth]{fig9_operating.pdf}
...
gradings or more. This is a transfer of an established construction to a paired
grading unit, not a claim to have invented a new statistical family.
...
conditional interval says while remaining in the units the reader can inspect
--- answers.
...
Strict matching, substring matching and numeric tolerance are \Graders{}
defensible rules; they disagree about \RegradeContested{} answers here,
...
scores \ForgedState{} invented in its place; the
...
answers disagrees about \RegradeContested{} of them, one of which moves the
...
Fig.~\ref{fig:flip-summary} collects the three pre-registered claims in one
view. The stored strict grader gives direction distances \SummaryHarmStrict{} and
\SummaryHelpStrict{} for the harm and help contrasts, while the verdict distance
for the long-tail text contrast is \SummarySigStrict{} as a raw per-contrast
distance. Repeating the same per-contrast calculation under the numeric-tolerant
grader changes these values to \SummaryHarmNumeric{}, \SummaryHelpNumeric{} and
\SummarySigNumeric{}, respectively. Holm survival remains a separate family-level
condition reported for the stored grader, with the numeric-tolerant family treated
separately in the regrade audit. The panel therefore conditions distances on the
grader without treating them as family-adjusted quantities.

\begin{figure}[tbp]
\centering
\includegraphics[width=\linewidth]{fig10_flip_regrade_summary.pdf}
...
accompany the manuscript source. Each artifact is recorded under an evidence ID
in a manifest, and the analysis code verifies the bound bytes before reading
them, so the build fails rather than silently reporting stale numbers. The knowledge graph and question sets are small
...
processed, and no new evaluation answers were generated; two local
instruction-model judges later scored the stored strings as a consistency
check. The questions concern publicly
...
including the paired differences, discordant splits, exact tests, regrade
record, and the handling of blocked comparisons --- are enforced by build-time checks that
stop the build. The evaluated responses were pre-existing records and were not
regenerated for this analysis. Responsibility for the content rests with the
authors.
