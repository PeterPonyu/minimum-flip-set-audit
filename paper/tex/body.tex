...
\(n\) grows. The same bound grid already records that share at the two
larger registered design sizes: \FragileGivenSigNTwoHundred{} at
\(n=\SimNTwoHundred{}\) and \FragileGivenSigNFiveHundred{} at
\(n=\SimNFiveHundred{}\). Among significant tables at \(n=\Questions{}\), Spearman correlation
...
\subsection{A labelled broader check on three public paired contrasts}

The \Contrasts{} contrasts above are one evaluation. A later, pre-declared
check applies the same two distances to a different family: \SecondEvalN{} public
paired binary tasks (ARC-Challenge, GSM8K, and Winogrande; $n=$
\SecondEvalNArc{}, \SecondEvalNGsm{}, and \SecondEvalNWino{}) on one
fixed model pair (\SecondEvalArmA{} and \SecondEvalArmB{}), chosen
before the per-item scores were read. The pair is not ranked here; the
table margins are not a leaderboard result, and this family does not
replace the \Contrasts{}-contrast reading.

\begin{table}[tbp]
\centering
\caption{\CapSecondEvalTable}
\label{tab:second-eval}
\small
\input{generated_table_second_eval}
\end{table}

Table~\ref{tab:second-eval} reports the two distances on each retained
contrast. Every contrast has direction distance at least
\SecondEvalKappaDirMin{} and verdict distance at least
\SecondEvalKappaVerMin{}, both above the prestated threshold of
\SecondEvalFragileMax{}. Single-record fragility at
$n=\SubsetN{}$ therefore does not transport to these pairs. The bound
\Contrasts{}-contrast findings are unchanged. The prestated
one-or-two-grading rule \SecondEvalFewGradeState{}.

The direction and the verdict still disagree. The \SecondEvalN{} gaps are
\SecondEvalGapArc{}, \SecondEvalGapGsm{}, and \SecondEvalGapWino{},
each at least the prestated gap of \DecoupledGap{}. That observation
transports; it is the same split this paper already reports on the
bound evaluation, now counted on this family only. Holm correction
over the family of \SecondEvalN{} leaves
\SecondEvalHolmSurvivors{} contrasts. The contrast that does not
survive is still \SecondEvalKappaVerWino{} gradings from a change of
verdict. Closed form and exhaustive search agree on all \SecondEvalN{} tables.
...
in the register of a finding. A labelled broader check on \SecondEvalN{} public pairs
of size \SecondEvalNArc{}, \SecondEvalNGsm{} and \SecondEvalNWino{} found
the same direction--verdict split (gaps \SecondEvalGapArc{},
\SecondEvalGapGsm{} and \SecondEvalGapWino{}) while every verdict
distance was at least \SecondEvalKappaVerMin{}; the
\Contrasts{}-contrast findings stay as they are.

\section{Scope of the conclusions}

The conclusions of this paper are about how an evaluation is reported, and
four boundaries fix their scope.
...
The fourth concerns a labelled broader check on \SecondEvalN{} public paired
tasks. It is a transport observation on that family only. It does not
enlarge, replace, or rank the \Contrasts{}-contrast findings, and it is
not a comparison of the two models whose stored item scores supplied
the tables.
...
A labelled broader check on \SecondEvalN{} public pairs of size
\SecondEvalNArc{}, \SecondEvalNGsm{} and \SecondEvalNWino{} found
direction--verdict gaps of \SecondEvalGapArc{}, \SecondEvalGapGsm{}
and \SecondEvalGapWino{}, while every verdict distance was at least
\SecondEvalKappaVerMin{}. The \Contrasts{}-contrast findings are
unchanged.
