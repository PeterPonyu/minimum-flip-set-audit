...
it needs \HelpK{} gradings to reverse. The one contrast that
...
but \RegradeContested{}. Table~\ref{tab:graders} gives the resulting aggregate
accuracies, and its usefulness is how little of them moves: eleven of its
twelve rows are identical across all three graders. The single disagreement is
kept at the aggregate level here; its row identity and answer strings remain in
the private evidence rather than the publication source. It is one of the
...
more. This is a transfer of an established construction to a paired grading
unit, not a claim to have invented a new statistical family.
...
One row deserves attention because it documents a decision rather than a
measurement. The panel built from the fixed measurement table is recorded and
re-hashed on every build, yet no figure reads it because its status places it
outside the registered analysis. Recording an artifact the analysis deliberately
declines to read may look redundant. It is the opposite: it is the only way the
decision to decline leaves a trace. An artifact that is simply never mentioned
is indistinguishable from one nobody noticed, whereas this row records that the
value existed, was seen, and was refused before it could enter an estimate.

Two limits keep the table honest. A checksum fixes bytes to a claim; it does not
make the bytes true, and nothing here certifies an artifact was correct when it
was written. And each entry's internal identifier and path are omitted
...
checksum in a manifest, and the analysis code verifies those checksums against the
