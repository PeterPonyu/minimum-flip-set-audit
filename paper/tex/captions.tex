...
the stored rule and the numeric-tolerant nested rule; only the multimodal
condition moves, and it moves by \RegradeImageFlip{} answer. Panel B gives the contrast those
...
Neither rule is endorsed here. The point is that the reported direction is
decided by which one is used.%
}
...
with \SimReps{} replicates per feasible cell. At the \(n\) of this evaluation
...
are the closed form checked against exhaustive search, not a new search.%
}
...
less-documented text-only contrast. Both columns are raw per-contrast distances;
Holm survival is a separate family-level condition and is reported for the
stored grader independently. The numeric-tolerant grader makes the harm contrast exactly level,
so its direction distance changes to \SummaryHarmNumeric{}; the help direction
remains \SummaryHelpNumeric{}, while the long-tail text verdict changes from
\SummarySigStrict{} to \SummarySigNumeric{}.
These are the same fixed paired tables and estimands as the main analysis,
conditioned on grader rather than a new test.
}
