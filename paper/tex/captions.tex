...
three panels are the same lattice over \SubsetN{} questions. The two on the
right are the same contrast, the same four counts and the same observed point,
read as two different claims: the direction survives \HelpK{} gradings and the
...
\newcommand{\CapAlpha}{%
The same family read against every threshold rather than one. The upper panel
...
last being fixed by how rarely the two conditions disagreed. The lower panel
...
The answers each one-grading conclusion is resting on, named. Where the smallest
number of gradings that would overturn a conclusion is one, every individual
grading that would suffice on its own can be enumerated, and this is that
enumeration for the two such conclusions in this study. Each block is one
conclusion, and its rows give the condition under which a grading would change,
how many questions qualify, and which. A reader can take any single question
listed, decide its grading for themselves, and the conclusion heading that block
stops holding.
