...
evidence_reader <- function(manifest, repo_root) {
  ...
  list(
    ...
    # The per-record evidence is one JSON object per line. Read as a frame rather
    # than as a list so that a row which is missing a field becomes a visible NA
    # instead of a shorter list that a later count would silently accept.
    records = function(id) jsonlite::stream_in(file(bound_path(id)), verbose = FALSE)
  )
}
...
public_evidence_note <- function(x) {
  replacements <- list(
    ...
    c("annotated with an internal label", "identified by a private marker"),
    c("internal label", "private marker"),
